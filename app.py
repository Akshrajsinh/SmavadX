...
def api_profile(username):
    ...
    try:
        ...
        if can_view_posts:
            rows = con.execute(
                """SELECT id, image, caption, created_at FROM posts
...
def api_create_post():
    ...
    file_name = save_upload(image, POST_DIR)
    if not file_name:
    ...
    try:
        cur = con.execute(
            "INSERT INTO posts(username, image, caption, location) VALUES (?, ?, ?, ?)",
            (current, file_name, caption, location),
        )
        con.commit()
...
def admin_posts():
    ...
    try:
        rows = con.execute(
            """SELECT p.id, p.username, p.image, p.caption, p.created_at,
