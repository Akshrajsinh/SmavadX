...
feedList.addEventListener('click', async (e) => {
  ...
  if (likeBtn) {
    ...
    } catch (err) { showToast('Could not update like'); }
    return;
  }
...
document.getElementById('postImageInput').addEventListener('change', (e) => {
  ...
  reader.onload = (ev) => {
    document.getElementById('postUploadText').innerHTML = `<img src="${ev.target.result}" alt="">`;
  };
...
document.getElementById('submitPostBtn').addEventListener('click', async () => {
  ...
  try {
    ...
    if (data.error) {
      ...
      document.getElementById('postUploadText').textContent = 'Click to choose a photo';
